...
func (a *configApplier) applyNotificationChannels() error {
	tx, cfg := a.tx, a.cfg
	msgs := a.msgs
	...
	if err != nil {
		return fmt.Errorf("applyConfig.listNotificationChannels: %w", err)
	}
	...
	for _, v := range notificationChannels {
		if _, ok := cfg.NotificationChannels[v.Slug]; !ok {
			...
			if err != nil {
				return fmt.Errorf("applyConfig.deleteNotificationChannelByID: %w", err)
			}
			continue
		}
	...
	for slug, v := range cfg.NotificationChannels {
		...
		if channel.Type == "slack" {
			...
			if err != nil {
				return fmt.Errorf("applyConfig.MarshalSlackNotificationDetails: %w", err)
			}
		...
		if _, ok := existingNotificationChannelSlugs[slug]; ok {
			...
			if err != nil {
				return fmt.Errorf("applyConfig.editNotificationChannel: %w", err)
			}
		} else {
			err := createNotification(tx, slug, channel.Name, channel.Type, details)
			if err != nil {
				return fmt.Errorf("applyConfig.createNotification: %w", err)
			}
		}
	}
